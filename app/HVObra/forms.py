...
class CrearHVOForm(FlaskForm):
    ...
    terminacion = StringField('Fecha de terminacion de la obra')
    fecha_socializacion = DateField('Fecha de socializacion', format='%d/%m/%Y', default=date.today)

    cuadro_base_datos = FileField('Cuadro base de Datos', validators=[Optional()])
    ...
    presupuesto = FileField('Presupuesto de Obra', validators=[Optional()])
    ...
    plan_manejo_ambiental = FileField('Plan de manejo ambiental', validators=[Optional()])
    registro_fotografico = FileField('Registro fotografico', validators=[Optional()])
    informe_diario = FileField('Formato de registro de informe diario', validators=[Optional()])
    acta_de_entrega = FileField('Acta de entrega de la Obra', validators=[Optional()])
